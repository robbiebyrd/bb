...
- **ELM327 vs STN adapters**: both speak the same line-oriented ASCII protocol (`internal/obd`: write `cmd\r`, read to `>` prompt; monitor streams frames until a stop byte). `elm327` uses the base AT command set (single `ATCF`/`ATCM` filter, `ATMA` monitor); `stn` (STN11xx, e.g. OBDLink MX+ = STN2120) adds the ST command set (multiple hardware pass filters via `STFAP`, `STM`/`STMA` monitor). The protocol library is transport-agnostic (`go.bug.st/serial`) and unit-tested against an in-memory `scriptedTransport` — no hardware needed. Shared CanConnection boilerplate + monitor/poll/hybrid orchestration live in `internal/connection/obdconn`; the `elm327`/`stn` packages are thin `Driver` adapters.
