...
| `INTERFACE_` (OBD: `elm327`\|`stn`) | `OBD_MODE` (`monitor`\|`poll`\|`hybrid`, default `monitor`), `OBD_PROTOCOL` (ELM327 `ATSP` selector; `6`=ISO15765 11/500 default, `7`=29/500, `0`=auto), `OBD_HW_FILTER` (comma-separated CAN IDs to pass in hardware), `OBD_PIDS` (comma-separated requests, e.g. `010C,010D`), `OBD_POLL_MS` (poll interval, default `1000`), `OBD_PORT_BAUD` (serial speed, default `115200`), `OBD_RESPONSE_FILTER` (auto-add OBD reply IDs to the HW filter in poll/hybrid, default `true`) |
| — | `MSG_BUFFER_SIZE`, `LOG_LEVEL`, `SIM_RATE` (ms, 0=unset), `SIM_RATE_MIN`, `SIM_RATE_MAX`, `CAN_INTERFACE_SEPARATOR`, `CONFIG_FILE` (optional JSON overlay) |
...
- **JSON config overlay**: `CONFIG_FILE=./config.json` is loaded by `config.Load` *after* env parsing and overlaid via `json.Unmarshal` into the already-populated `Config` — only keys present in the file are applied (env/defaults remain for the rest), so precedence is defaults < env < JSON file < CLI flags. Struct json tags are `camelCase`; nesting follows the `envPrefix` groups (so `INTERFACE_0_OBD_MODE` ↔ `interfaces[0].obd.mode`). A JSON `interfaces` array replaces env-derived interfaces wholesale; `ValidateInterfaces` re-checks each has a name since the json path bypasses the env `required` tag. Per-interface OBD options live in a nested `OBDOptions` sub-struct (`envPrefix:"OBD_"`), so the env vars are `INTERFACE_<n>_OBD_{MODE,PROTOCOL,HW_FILTER,PIDS,POLL_MS,PORT_BAUD}`.
